...
- **Codex local env sync:** Cursor's full local env file lives at `/Users/morgees/GrantPilot/.env.local`. For Codex runs, copy that file into this repo's `.env.local` before local automation/API work, without printing secret values. After copying, remove accidental spaces after `=` in key assignments if needed, then verify by key names/boolean presence only. Never commit `.env.local`.
